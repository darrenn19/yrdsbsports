# Tier 1 Varsity Girls Slo Pitch Schedule
import requests
...
def scrape_schedule():
    url = "http://yraa.com/src/schedule.php?division=106"  # Schedule over next 7 days
    ...
    # Save JSON
    with open("slo-pitch/t1/t1schedule.json", "w", encoding="utf-8") as f:
        json.dump({"schedule": schedule}, f, indent=4, ensure_ascii=False)

    print(f"Saved {len(schedule)} entries to slo-pitch/t1/t1schedule.json")
