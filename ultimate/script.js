...
// ------------------ Standings ------------------
fetch("ultimatet1standings.json")
  ...
  .then((data) => {
        ...
        for (const team of grouped[div]) {
          ...
          out += `
            ...
            team.PTS ?? ""
              ...
              <td class="${sortColumn === "SPTS" ? "active-col" : ""}">${
            team.SPTS ?? ""
          }</td>
    ...
    fetch("ultimatet1standings.json", { method: "HEAD" }).then((res) => {
...
// ------------------ Standings ------------------
fetch("ultimatet2standings.json")
  .then((response) => response.json())
  .then((data) => {
    const placeholder = document.querySelector("#data-output");
    let standings = data.standings || [];
    ...
    fetch("ultimatet2standings.json", { method: "HEAD" }).then((res) => {
      const lastModified = res.headers.get("last-modified");
      if (lastModified) {
        document.getElementById("last-updated").textContent =
          "Standings last updated: " + new Date(lastModified).toLocaleString();
